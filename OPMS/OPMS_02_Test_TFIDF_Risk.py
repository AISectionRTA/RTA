...
datapath = os.path.join(
    os.path.join(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Data'), 'Risk Analysis'),
    'Risk analysis.csv')
...
def tsne_plot3d(model, lexicon):
    ...
    for word2 in lexicon[len(lexicon) - size - 1:len(lexicon) - 1]:
        if word2[0] in model.wv.vocab:
            tokens2.append(model[word2[0]])
            labels2.append(word2[0])

    tsne_model2 = TSNE(perplexity=40, n_components=3, init='pca', n_iter=2500, random_state=23)
    ...
    for value2 in new_values2:
        ...
        Y2.append(value2[1])
        Z2.append(value2[2])
    for x2, y2, z2, labl2 in zip(X2, Y2, Z2, labels2):
        dictList.append(
            dict(showarrow=False, x=x2, y=y2, z=z2, text=labl2, xanchor="left", xshift=5)
        )
    for value in new_values:
        X.append(value[0])
        Y.append(value[1])
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter3d(x=X2, y=Y2, z=Z2, mode="markers", name="z", ))
